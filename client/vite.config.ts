  ...
  plugins: [
    ...
    VitePWA({
      ...
      workbox: {
        ...
        // ERPRAY-PATCH: 4 MB -> 6 MB. vega + vega-lite + vega-embed (the chart
        // patch) pushed the vendor chunk to 4.4 MB, over the previous limit --
        // the build FAILED outright at the Workbox precache-manifest step
        // ("Configure workbox.maximumFileSizeToCacheInBytes"), which only
        // surfaces in a full `docker compose build`, never in `tsc --noEmit`.
        // 6 MB leaves headroom rather than re-tripping this the next time a
        // dependency grows.
        maximumFileSizeToCacheInBytes: 6 * 1024 * 1024,
        navigateFallbackDenylist: [/^\/oauth/, /^\/api/],
      },
      includeAssets: [],
      // ERPRAY-PATCH: the PWA install prompt / home-screen label / splash
      // background — a branding surface `AuthLayout.tsx`'s Wordmark patch does
      // NOT reach, because this manifest is generated by vite-plugin-pwa from
      // this config, not from a static file.
      manifest: {
        name: 'ERPray',
        short_name: 'ERPray',
        display: 'standalone',
